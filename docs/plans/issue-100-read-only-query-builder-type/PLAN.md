...
| `probes/03-readonly-query.ts` | **V1–V6, V9** — §6.1's declarations verbatim (minus `export` and the JSDoc, neither of which affects type resolution) plus every §8.1 assertion, against a real `withSchema` repo and a W ≠ T repo. Keep the declarations in sync with §6.1: this file is the evidence §6 compiles as written | prints (0 diagnostics ⇒ pass) |
