...
| Write interceptors (`registerWriteInterceptor`)  | Sibling writes **guaranteed** to commit in the triggering write's atomic boundary. Write-only interceptors stage into a `WriteBatch`; a declared `read` phase promotes the repository to a transaction. Single-document writes and the `*InTransaction` helpers always run them; fixed batches and `query().update()`/`delete()` run write-only ones in chunks of `floor(500 / writes-per-document)` documents (counted from the writes actually staged, not the number of interceptors) and **throw** for a read-capable one; `bulkWrite` and both recursive deletes **throw** whenever any is registered (no shared boundary, and no waiver — a guarantee is not a notification). `{ withMetadata: true }` throws under transaction mode. Additive: nothing changes until one is registered. |
